...
def main():
    ...
    start_message = "Sparse DBSCAN clustering run has started."
    ...
    # --- Run Sparse DBSCAN Strategy ---
    pipeline = ClusteringPipeline(config)
    result = pipeline.run_sparse_dbscan(data_array)
    ...
    pipeline.save_results(result, "sparse_dbscan_all_result.json")
    logging.info("Result saved to sparse_dbscan_all_result.json")
    ...
    message = (
        "Sparse DBSCAN clustering completed successfully.\n"
...
if __name__ == "__main__":
    try:
        ...
        error_message = (
            "Sparse DBSCAN clustering pipeline encountered an error:\n" +
