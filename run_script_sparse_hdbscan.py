...
def main():
    ...
    start_message = "Sparse HDBSCAN clustering run has started."
    ...
    # --- Run Sparse HDBSCAN Strategy ---
    pipeline = ClusteringPipeline(config)
    result = pipeline.run_sparse_hdbscan(data_array)
    ...
    pipeline.save_results(result, "sparse_hdbscan_all_result.json")
    logging.info("Result saved to sparse_hdbscan_all_result.json")
    ...
    message = (
        "Sparse HDBSCAN clustering completed successfully.\n"
...
if __name__ == "__main__":
    try:
        ...
        error_message = (
            "Sparse HDBSCAN clustering pipeline encountered an error:\n" +
